...
def get_options():
    ...
    IO.add_argument('--panaroo_graph',################3
                    required = True,
                    help='the path to the final_graph.gml generated by ggCaller. ')
...
def main():
    ...
    if options.panaroo_graph is not None:
        try:
            panaroo_graph = nx.read_gml(options.panaroo_graph)
            print(f"Successfully loaded Panaroo graph from {options.panaroo_graph}")
            print(f"Panaroo graph has {panaroo_graph.number_of_nodes()} nodes and {panaroo_graph.number_of_edges()} edges.")
        except Exception as e:
            print(f"Error loading Panaroo graph: {e}")
            raise#############
    ...
    # query unitigs in previous saved ggc graph
        ...
        if options.data is None:
            ...
            sys.exit(1)
        search_graph(graph, options.panaroo_graph, options.graph, options.colours, options.query, options.data, output_dir, options.query_id,
                     options.threads)
        print("Finished.")
        sys.exit(0)
